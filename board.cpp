...
const char &Board::getPlayerLetter() const { return playerLetter; }

const char &Board::getComputerLetter() const { return computerLetter; }

const char &Board::getCurrentPlayer() const { return currentPlayer; }

void Board::setCurrentPlayer(char player) { currentPlayer = player; }

bool Board::isEmpty(int place) const { return board[place] == ' '; }
...
bool Board::isWinner(const char player) const {
    return ((board[6] == player && board[7] == player &&
             board[8] == player) || // across the top
            (board[3] == player && board[4] == player &&
             board[5] == player) || // across the middle
            (board[0] == player && board[1] == player &&
             board[2] == player) || // across the bottom
            (board[6] == player && board[3] == player &&
             board[0] == player) || // down the left side
            (board[7] == player && board[4] == player &&
             board[1] == player) || // down the middle
            (board[8] == player && board[5] == player &&
             board[2] == player) || // down the right side
            (board[6] == player && board[4] == player &&
             board[2] == player) || // diagonal
            (board[8] == player && board[4] == player &&
             board[0] == player)); // diagonal
}
...
int Board::evaluate(const char &player) const { return 0; }
